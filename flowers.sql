...
SELECT cZone AS 'Cool Zone' , COUNT(*) AS 'Number of Flowers' 
    FROM flowersInfo GROUP BY cZone;


-- c) common names of the plants that have delivery sizes less than 5.
SELECT comName AS 'Common Name: delSize < 5' 
    FROM FlowersInfo A INNER JOIN Deliveries B 
    ON A.deliver = B.id WHERE B.delSize < 5;


-- d) common names of the plants that require full sun (i.e., sun needs contains ‘S’).
SELECT comName AS 'Common Name: requires full Sun' 
    FROM flowersInfo WHERE 
    sunNeeds LIKE 'Sto%' OR sunNeeds LIKE 'S';


-- e) all delivery category names order alphabetically (without repetition).
SELECT categ AS 'Delivery Category Names' 
    FROM Deliveries GROUP BY categ ORDER BY categ;


-- f) the exact output (note that it is order by Name):
SELECT A.comName AS 'Name',B.lowerTemp 
                 AS 'Cool Zone (low)',B.higherTemp 
                 AS 'Cool Zone (high)',C.categ 
                 AS 'Delivery Category' 
    FROM FlowersInfo A INNER JOIN Zones B ON A.cZone = B.id,Deliveries C 
    WHERE C.id = A.deliver ORDER BY A.comName;
...
SELECT comName AS 'Common Name',latName 
               AS 'Latin Name' 
               FROM flowersInfo A 
               WHERE A.hZone = @PChzone AND A.comName != 'Pink Caladiums';
...
SET @Totalplants := (SELECT COUNT(deliver) FROM flowersInfo A 
                        WHERE deliver != 5 AND deliver != 6);
SET @MINdelSize := (SELECT ROUND(MIN(delSize),1) FROM Deliveries);
SET @MAXdelSize := (SELECT ROUND(MAX(delSize),0) FROM Deliveries);
SET @AVGdelSize := (SELECT ROUND(AVG(B.delSize),2) 
                        FROM FlowersInfo A INNER JOIN Deliveries B 
                        ON A.deliver = B.id WHERE deliver != 5 AND deliver != 6);
SELECT @Totalplants AS 'Total',
       @MINdelSize AS 'Min',
       @MAXdelSize AS 'Max',
       @AVGdelSize AS 'Average'; 
...
SELECT A.categ AS 'Category',B.comName 
               AS 'Name' FROM Deliveries A INNER JOIN 
               flowersInfo B ON A.id = B.deliver ORDER BY A.categ,B.comName;